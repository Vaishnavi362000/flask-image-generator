...
function SignInPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        window.localStorage.href = '/dashboard'; // Use navigate instead of window.location
      }
    } catch (error) {
  ...
  const handleGoogleSignIn = useGoogleLogin({
    onSuccess: async (response) => {
      ...
      console.log("ID Token:", response.credential);  // Use response.credential for the ID token
  
      try {
        // Send the ID token to your backend
        const res = await api.post('/auth/google-login', {
          token: response.credential  // Send the ID token, not the access token
        });
